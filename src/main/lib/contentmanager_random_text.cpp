...
ContentManager_Random_Text::ContentManager_Random_Text(std::unique_ptr<Protocol> _protocol, CommonHeaders &_headerHandler) :
        generator(std::chrono::system_clock::now().time_since_epoch().count()),
        chars(32, 127), distribution(nullptr),
        ContentManagerBase(std::move(_protocol), _headerHandler) {
}
