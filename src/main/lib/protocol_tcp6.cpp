...
ProtocolTCP6::ProtocolTCP6() : host(Host::ALL_INTERFACES), type(ProtocolType::NONE),
        socket(-1), state(ProtocolState::CLOSED) {
}

ProtocolTCP6::ProtocolTCP6(int newSocket, socklen_t len, const struct sockaddr * addr) : host(len, addr, false),
        type(ProtocolType::SERVER_CLIENT), socket(newSocket), state(ProtocolState::OPEN) {
}

ProtocolTCP6::ProtocolTCP6(ProtocolTCP6 && other) : host(other.host),
...
ProtocolTCP6::ProtocolState ProtocolTCP6::getState() {
    std::unique_lock<std::mutex> lck(lock);
    return state;
}

bool ProtocolTCP6::isReady(const ProtocolState & expected, int timeoutInMilliseconds) {
    std::unique_lock<std::mutex> lck(lock);
    pollfd fd;
    fd.fd = socket;
    fd.revents = 0;
...
ProtocolTCP6::ProtocolType ProtocolTCP6::getType() {
    std::unique_lock<std::mutex> lck(lock);
    return type;
}
